...
* имеет уникальный `ID` (вычисляется автоматически / можно задать вручную);
...
Пакеты наследуют `NetPacket<H>` и должны реализовать методы:

* `write(BinaryOutputStream stream)` - для сериализации в поток
	
* `read(BinaryInputStream stream)` - для десериализации из потока
	
* `handle(H handler)` - для логики обработки.
...
Обычно не нужна напрямую - `PacketReader` сам генерирует через рефлексию, если зарегистрировать класс пакета.
